...
class Environment():
    def __init__(self, num_features:int, granularity:int, start_date:str, end_date:str=None):
        ...
        self.state_space = PriceHistory(num_features=self.num_features, granularity=self.granularity, start_date=self.start_date, end_date=self.end_date)
        self.period = 0
    ...
    def calculate_reward(self, state_action:dict)-> float:
        ...
        y_t = np.reciprocal(X_t[0, -2, :])      #np.ones_like(state_action["state_t"][0, -2:, :]) / state_action["state_t"][0, -2:, :]   
        w_t_prime = (np.multiply(y_t, w_t_1)) / np.dot(y_t, w_t_1)
        mu_t = self.commision_rate_selling * sum(np.abs(w_t_prime - state_action["acton_t"])) 
    ...
    def reset(self):
        ...
        start_action[0] = 1
        start_state = (self.state_space.normalized_price_matrix(self.period), np.empty(self.state_space.num_assets))
    ...
    def step(self, action): 
        ...
        curr_states_action = {"state_t": (self.state_space.normalized_price_matrix(self.period), self.action_buffer), "action_t": action} # For the rewards
        ...
        done = self.period + self.state_space.num_periods == self.state_space.data_matrix[0].shape[0] # True if end of sequence
