    ...
    * prices are independent of a
    ...
    def _check_dates(self) -> None:
        data_points = count_granularity_intervals(
            self.start_date, self.end_date, self.granularity
        )
        assert (
    ...
    def get_list_of_npm(self) -> List[pd.DataFrame]:
        return [
            self.normalized_price_matrix(idx=i)
            for i in range(1, self.data_matrix[0].shape[0] - self.num_periods)
        ]

    def __efficent_all(
        self, cash_bias: bool = True
    ) -> List[pd.DataFrame]:  # returns List[normalized_price(close),] for all assets
    ...
    def __fill_sequence_in_column(self, df: pd.DataFrame, column_name: str):
        ...
        first_value_idx, last_value_idx = self.__get_nan_sequence_from_column(
            nan_indices
        )
    ...
    def __fill_nan(self, data: Tuple[pd.DataFrame]) -> Tuple[pd.DataFrame]:
        ...
        while (
            closes.isnull().values.any()
            or highs.isnull().values.any()
            or lows.isnull().values.any()
        ):
    ...
    def __normalized_feature_matrix1(
        self, idx: int, feature_matrix: pd.DataFrame
    ) -> np.array:
        """
    ...
    def __normalized_feature_matrices(self, idx: int) -> np.array:
        ...
        return [
            (feature_matrix.iloc[idx - self.num_periods : idx, 1:] / v_t).to_numpy()[
                np.newaxis, ...
            ]
            for feature_matrix in self.filled_feature_matrices
        ]
    ...
    def normalized_price_matrix(self, idx: int) -> np.array:
        ...
        assert (
            idx <= self.data_matrix[0].shape[0]
        ), f"total length {self.data_matrix[0].shape[0]} but idx={idx}"
...
class FedPriceData(Dataset):
    ...
    def __len__(self) -> int:
        return (
            len(self.filled_feature_matrices[0]) - self.num_periods - self.pred_len + 1
        )
