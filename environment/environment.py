...
class Environment:
    ...
    def calculate_reward(self, state_action: dict) -> float:
        ...
        if self.args.use_numeraire:
            y_t = np.reciprocal(X_t[0, -2, 1:])  # Correct
            y_t = np.concatenate((np.ones((1)), y_t))
        else:
            y_t = np.reciprocal(X_t[0, -2, :])  # Correct
    ...
    def calculate_relative_price_episode(self) -> np.array:
        ...
        last_closes = self.state_space[
            -(self.state_space.num_periods + self.state_space.pred_len + 5)
        ][1]
        if self.scale_state:
