            ...
            if not existing_data.empty:
                ...
                for data in [data_before, data_after]:
                    if data is not None:
                        ...
                        logger.info(f"Inserting data for period from {start_date_} to {end_date_} into table {table_name}")
    ...
    def get_coin_data(self, coin: str, granularity, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        with sqlite3.connect(self.database_path) as connection:
            ...
            if s_date_dt < first_date and e_date_dt > last_date:
                logger.info(f"Could only fetch data from {start_date_str} to {end_date_str}. Consider to fill the database.")
            elif s_date_dt < first_date and e_date_dt < last_date:
                logger.info(f"Could only fetch data from {start_date_str} to {end_date}. Consider to fill the database.")
            elif s_date_dt > first_date and e_date_dt > last_date:
                logger.info(f"Could only fetch data from {start_date} to {end_date_str}. Consider to fill the database.")
            elif s_date_dt > first_date and e_date_dt < last_date:
                logger.info(f"Could only fetch data from {start_date} to {end_date}. Consider to fill the database.")
